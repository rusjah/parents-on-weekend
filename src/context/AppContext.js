...
export const AppProvider = ({children}) => {
  ...
  function generateOptions (userOptions) {
    let options = {};
    PETS.map (el => {
      userOptions.pets.includes (el.nameId)
        ? (options[`${el.nameId}`] = true)
        : (options[`${el.nameId}`] = false);
    });
    CHILDREN.map (el => {
      userOptions.children.includes (el.nameId)
        ? (options[`${el.nameId}`] = true)
        : (options[`${el.nameId}`] = false);
    });
    return options;
  ...
  async function getReferenseFile (userFile, fileFolder, fileName) {
    const fileUploadResponse = await Backendless.Files.upload (
      userFile,
      fileFolder,
      fileName
    );
  ...
  async function registration (registretedUser, userOptions) {
    ...
    try {
      ...
      if (newUser.photo) {
        const fileReference = await getReferenseFile (
          newUser.photo,
          'images',
          true,
          `avatar-${Math.random () * 10}`
        );
        const photo = {
          tag: 'avatars',
  ...
  async function getCurrentUser () {
    try {
      ...
      const user = await Backendless.Data
        .of ('Users')
        .findById (refCur.objectId, {
          relations: [`optionsId`],
        });
      setCurrentUser (user);
  ...
  async function saveReview (review) {
    try {
      ...
      const relation = await Backendless.Data
        .of ('reviews')
        .addRelation (userRel, 'usersId', [optRel]);
      const savedReview = await Backendless.Data
        .of ('reviews')
        .findById (send.objectId, {
  ...
  async function filterReviw () {
    if (togleReview) {
      ...
      const filteredRev = reviews.filter (el => el.ownerId === curUsr.objectId);
  ...
  async function deleteReview (review) {
    await Backendless.Data.of ('reviews').remove (review);
    getAllReviws ();
    filterReviw ();
  }
